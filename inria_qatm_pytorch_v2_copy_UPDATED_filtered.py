...
# New detection function using slope filter example
def detect_and_filter(model, template, image, expected_slope, slope_tolerance=0.1, thresh=0.7):
    scores = run_one_sample(model, template, image)
    w = template.size()[-1]
    h = template.size()[-2]
    boxes = nms(scores[0], w, h, thresh=thresh)
    points = [((box[0][0]+box[1][0])//2, (box[0][1]+box[1][1])//2) for box in boxes]
    filtered_points = filter_by_slope(points, expected_slope, tolerance=slope_tolerance)
    filtered_boxes = [box for box, pt in zip(boxes, points) if pt in filtered_points]
    return filtered_boxes, filtered_points
